...
const authResult = validateAuthRequest(
  { username: 'test', password: '123456' },
  'login',
);
console.log(
  `   ✓ Auth validation: ${
    authResult.error ? 'FAILED - ' + authResult.error.message : 'PASSED'
  }`,
);
...
console.log(
  `   ✓ Weather endpoint validation: ${
    weatherResult.error ? 'FAILED' : 'PASSED'
  }`,
);

const coordResult = validateCoordinates(52.52, 13.41);
console.log(
  `   ✓ Coordinate validation: ${coordResult.valid ? 'PASSED' : 'FAILED'}`,
);
...
console.log(
  `   ✓ SQL Injection Protection: ${
    sqlSanitized.includes('DROP') ? 'FAILED' : 'PASSED'
  }`,
);
